...
import {
  ...
  Phone, Mail, Building2, Calendar, MapPinned,
  ChevronDown, ArrowRight, ArrowLeft, IndianRupee
} from 'lucide-react'
import { motion, AnimatePresence } from 'framer-motion'
import { submitApplication } from '../utils/jobUtils'
...
export default function GeneralApplicationForm({ jobs = [], onClose }) {
  ...
  const [form, setForm] = useState({
    ...
    expectedCTC: '', currentCity: '', workMode: '', jobProfileSelect: '', jobProfile: ''
  })
  ...
  const jobOptions = [...jobs.map(j => ({ v: j.title, l: j.title })), { v: 'Other', l: 'Others' }]
  ...
  const validateStep = (currentStep) => {
    ...
    if (currentStep === 0) {
      ...
      if (form.jobProfileSelect === 'Other' && !String(form.jobProfile || '').trim()) e.jobProfile = 'Please specify the job role'
  ...
  const calculateProgress = () => {
    let required = ['fullName', 'email', 'phone', 'jobProfileSelect', 'experience', 'currentlyWorking', 'earliestJoinDate', 'expectedCTC', 'currentCity', 'workMode']
    if (form.currentlyWorking === 'yes') required.push('currentCompany')
    if (form.jobProfileSelect === 'Other') required.push('jobProfile')
    
    let filled = required.filter(f => form[f] && form[f].trim() !== '').length
    if (cvFile) filled += 1
    const total = required.length + 1
    return Math.max(5, (filled / total) * 100)
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Do not append jobId for general application
      
      const submissionData = { ...form }
      // If they selected a specific job from dropdown, set jobProfile to that
      if (submissionData.jobProfileSelect !== 'Other') {
        submissionData.jobProfile = submissionData.jobProfileSelect
      }

      Object.entries(submissionData).forEach(([k, v]) => {
        if (v && k !== 'jobProfileSelect') fd.append(k, v)
      })
    ...
    return (
      <div className="text-center py-16 px-6 bg-white rounded-2xl shadow-sm border border-gray-100">
        ...
        <p className="text-gray-600 max-w-md mx-auto mb-2">
          Thank you for applying. We've received your profile and sent you a confirmation email.
        </p>
        <p className="text-gray-500 text-sm mb-10">Our team will review your details and reach out if there's a match.</p>
  ...
  return (
    <div className="w-full bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100">
      {/* Progress indicators */}
            ...
            >
              <FormSection title="Personal Information">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <Field label="Job Profile Applying for? *" error={errors.jobProfileSelect}>
                    <CustomSelect value={form.jobProfileSelect} onChange={v => set('jobProfileSelect', v)} options={jobOptions} placeholder="Select job role" />
                  </Field>
                  {form.jobProfileSelect === 'Other' && (
                    <Field label="Specify Job Role *" error={errors.jobProfile}>
                      <Input placeholder="e.g. Graphic Designer" value={form.jobProfile} onChange={v => set('jobProfile', v)} />
                    </Field>
                  )}
              ...
              <FormSection title="Resume / CV *">
                <Field error={errors.cv || cvError}>
                  <div
                    ...
                    onClick={() => document.getElementById('cv-upload-general').click()}
                  ...
                  >
                    ...
                    <input id="cv-upload-general" type="file" accept=".pdf,.doc,.docx" className="hidden" onChange={handleFile} />
                  </div>
                </Field>
              </FormSection>
            </motion.div>
      ...
      <div className="flex items-center gap-4 pt-4 mt-4 border-t border-gray-100">
        {step > 0 && (
          <button
            type="button"
            onClick={handlePrev}
            className="btn-secondary flex items-center gap-2 bg-gray-100 px-4 py-2 rounded-xl hover:bg-gray-200"
          >
...
function CTCField({ label, error, value, onChange }) {
  ...
  const handleChange = (e) => onChange(formatNumber(e.target.value));
